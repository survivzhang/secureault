...
*2026-07-16* · Covers plan Day 17b.
...
### 💡 What I learned
- **The three-state pattern:** every network fetch can be *loading*, *error*, or *data*.
...
- **Empty ≠ loading:** an empty vault (`passwords.length === 0`) is a *success* with no
...
- **`===` vs `==`:** always use `===` (strict — compares type *and* value, no hidden
  type coercion).
...
- **`onClick` trap:** must be `onClick={() => handleDelete(p.id)}` (a function to run *later*),
...
- **Backend `app.delete("/passwords/:id", authenticateToken, ...)`:**
  ...
  - `WHERE` only *selects which row* — `DELETE` removes the **whole row**, not just those columns.

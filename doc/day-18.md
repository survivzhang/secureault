...
_2026-07-16_ · Completes plan Day 18 (the `/dashboard` route + `useNavigate` were done back in day-17; this closes the guard).
...
- **The `children` prop:** whatever JSX you nest between a component's tags becomes its
  ...
  Think "box": ProtectedRoute _receives_ the element and decides whether to render it.
...
- **`<Navigate to="/login" replace />`** redirects by _rendering_ (not by calling a function).
...
- **`return` + JSX parens** are optional — only needed when JSX starts on the line _below_
...
- **Day 19** — Axios 401 redirect: in `services/api.ts` response interceptor, the 401 branch
  ...
  (pairs perfectly with today's guard — handles the _expired-token-mid-session_ case).
